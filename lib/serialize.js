const { proto, jidDecode, downloadContentFromMessage, getContentType,generateForwardMessageContent,generateWAMessageFromContent } = require("@adiwajshing/baileys")
...
let	{ isUrl } = require("./function")
let	phonenumber = require("awesome-phonenumber")
let	{ toOpus, toAudio, convert, convert2 } = require("./convert")
let	{ toPTT, toAudio: toAudio2 } = require("./converter")
const	cmd = {
		1: [
			"-fs 1M",
			"-vcodec",
...
conn.copyNForward = async (jid, message, forceForward = false, options = {}) => {
	...
	if (msg.message) {
      ...
      const send = await quot.forward(to ? to : msg.from)
    }
		} catch (e) {
			msg.quoted = null;
		}
