...
const antiluar = async(conn, msg) => {
  ...
  let local = localOnly.includes(msg.id);
	...
	for (let i of participant) {
	  if (msg.action == "add"){
	    if(!local && i.startsWith('881') || i.startsWith('991') || i.startsWith('212') || i.startsWith('91') || i.startsWith('92') || i.startsWith('1') || i.startsWith('60') || i.startsWith('65') || i.startsWith('265')) {
	     let luar = `*Sorry @${i.split("@")[0]}⁩, este grupo es solo para indonesios y serás eliminado automáticamente.*`
