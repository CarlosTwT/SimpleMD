...
module.exports = {
	...
	alias: ["s","sticker",],
	category: "converter",
	desc: "Crea una pegatina a partir de una imagen o un vídeo",
		...
		try {
			if ((isMedia && !msg.message.videoMessage) || isQImg) {
				...
				let ext = /image/.test(quoted.message.documentMessage.mimetype)
					...
					? { isVideo: true }
					: null;
				if (!ext) return await msg.reply("Documento mimetype desconocido");
				buffer = await quoted.download();
				stickerBuff = await sticker(buffer, { ...ext, withPackInfo: true, packInfo, cmdType: "1" });
				await conn.sendMessage(from, { sticker: stickerBuff }, { quoted: msg});
			} else {
				await msg.reply(`Responde imagen`);
			}
			(buffer = null), (stickerBuff = null);
		} catch (e) {
			console.log(e);
			await msg.reply("Error al crear la etiqueta\n\n" + e);
		}
	},
};
